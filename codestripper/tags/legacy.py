...
from codestripper.tags.replace import calculate_replacement
from codestripper.tags.tag import RangeTag, RangeOpenTag, RangeCloseTag, TagData, SingleTag
from codestripper.tags.utils import calculate_replacement_whitespace
...
class LegacyOpenTag(RangeOpenTag):
    ...
    def execute(self, content: str) -> Union[str, None]:
        # Legacy command "Start Solution::replacewith::" does both remove and replace
        # To support that command we need to check if we need to replace
        # The matched regex has the index of the regex at the end
        return calculate_replacement_whitespace(content, "::", self)
